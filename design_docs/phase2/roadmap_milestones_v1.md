...
- `P2-M2c` 补齐 `procedure_spec` 自身治理；该能力保留为后续实验地基，但不再牵引 P3 主线。
- `P2-M2d` 只做 ADR 0060 的最薄 ledger 写入预备；它为 `P2-M3` 的 identity / visibility policy 提供落点，但不提前完成 memory migration。
- `P2-M3` 是 P2 收口点：它冻结 principal、visibility 与 memory ledger/read path 边界；完整 Shared Companion、外部协作表面和 self-evolution workflow 均移出 P2。Phase 3 当前草稿改为 daily-use 能力补完。
...
- 完整 self-evolution workflow 不作为 Phase 3 当前主线；如未来重启，必须重新进入计划审批，不得沿用旧候选口径直接开工。
...
- Self-Evolution CLI Demo：
  - 不再作为 P2 milestone；截至 2026-04-19，也不作为 Phase 3 当前主线。`P2-M2c` / `P2-M2d` 只处理必要前置地基。
- Slack / 群聊：
...
| v1 | 2026-04-19 | Phase 3 当前草稿调整为 daily-use 能力补完；self-evolution workflow 从 P3 主线降级 | design_docs/phase3/p3_daily_use_roadmap_draft.md |
