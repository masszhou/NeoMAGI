...
def get_model(msg: Dict[str, Any]) -> str:
    m = msg.get("message")
    if isinstance(m, dict):
        return _normalize_model_name(m.get("model"))
    return _normalize_model_name(None)

def get_usage_details(msg: Dict[str, Any]) -> Optional[Dict[str, int]]:
    m = msg.get("message")
...
def emit_turn(langfuse: Langfuse, session_id: str, turn_num: int, turn: Turn, transcript_path: Path) -> None:
    ...
    model = get_model(last_assistant)
